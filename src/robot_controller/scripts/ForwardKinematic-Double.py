#!/usr/bin/python3
"""
forward_kinematics_double_track.py
ใช้ Double-Track Model ในการคำนวณ odometry จาก wheel speeds
โดยแยกคำนวณความเร็วของล้อซ้ายและขวา
"""
...
class ForwardKinematicsDoubleTrack(Node):
    def __init__(self):
        super().__init__('forward_kinematics_double_track')
        # Parameter: track width (W)
        ...
        self.odom_pub = self.create_publisher(Odometry, '/odom_double_track', 10)
...
def main(args=None):
    ...
    node = ForwardKinematicsDoubleTrack()
    rclpy.spin(node)
    node.destroy_node()
    rclpy.shutdown()
