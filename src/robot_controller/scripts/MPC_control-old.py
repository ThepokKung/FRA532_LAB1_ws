...
Dynamics (linearized about the current state, assuming small steering angles):
    x[k+1]   = x[k] + dt * v[k] * cos(phi0)
    y[k+1]   = y[k] + dt * v[k] * sin(phi0)
    v[k+1]   = v[k] + dt * a[k]
    phi[k+1] = phi[k] + dt * (v0 * delta[k] / L)

Here, phi0 and v0 are taken from the current state and treated as constants over the horizon.
The MPC minimizes a quadratic cost of tracking error and control effort.
After solving, the computed steering angle delta is converted to angular velocity using:
    ω = v_ref * tan(delta) / L
and then published via cmd_vel.
Reference trajectory is generated using the current target waypoint (constant over the horizon)
โดย target waypoint นี้จะใช้ค่า x, y, และ yaw จากไฟล์ YAML
"""
...
class MPCPathTracking(Node):
    def __init__(self):
        ...
        # Load path from YAML file (expected format: list of waypoints with keys 'x', 'y', and 'yaw')
        ...
        self.Q = np.diag([5.0, 5.0, 1.0, 5.0])    # State tracking cost
        self.R_mat = np.diag([0.1, 0.05])              # Control effort cost
        self.Qf = self.Q.copy()                       # Terminal state cost
        
        # Input constraints
        self.a_max = 1.0          # Maximum acceleration (m/s²)
        self.delta_max = 0.5236   # Maximum steering angle (rad) ≈ 30°
        
        # Reference speed (m/s)
        self.v_ref = 5.0  # Adjust as needed
        
        # Waypoint threshold for switching target (m)
        self.declare_parameter('waypoint_threshold', 0.5)
        self.waypoint_threshold = self.get_parameter('waypoint_threshold').value
        self.current_target_index = 0
        
        # Initial state (will be updated from odometry): [x, y, v, phi]
        self.x0 = None
    ...
    def odom_callback(self, msg: Odometry):
        ...
        # Calculate distance error to current target waypoint
        ...
        # Generate a constant reference trajectory using the current target waypoint.
        # Here, we use the provided yaw from the waypoint as the target orientation.
        ref_traj = self.generate_reference_trajectory_constant()
        ...
        # Use the first control input: u0 = [a, delta]
        a_cmd = u_opt[0, 0]
        delta_cmd = u_opt[1, 0]
        
        # Convert steering angle delta to angular velocity using the bicycle model:
        # ω = v_ref * tan(delta) / L
        angular_velocity_cmd = self.v_ref * math.tan(delta_cmd) / self.L
        
        self.get_logger().info(f"🎯 Target Position: x={tx:.2f}, y={ty:.2f}")
        self.get_logger().info(f"🚗 MPC Command: a = {a_cmd:.2f}, δ = {delta_cmd:.2f} (converted to ω = {math.degrees(angular_velocity_cmd):.2f}°/s)")
    ...
    def generate_reference_trajectory_constant(self):
        """
        Generate a constant reference trajectory for the next N+1 steps using the current target waypoint.
        Each reference state: [x_target, y_target, v_ref, phi_target]
        Here, phi_target is taken directly from the waypoint's 'yaw' field.
        Returns a numpy array of shape (4, N+1).
        """
        if self.current_target_index >= len(self.path):
            wp = self.path[-1]
        else:
            wp = self.path[self.current_target_index]
        x_target = wp['x']
        y_target = wp['y']
        # Use the provided yaw field as the desired orientation
        phi_target = wp.get('yaw', 0.0)
        ref_state = np.array([x_target, y_target, self.v_ref, phi_target])
        # Repeat this reference state for all steps in the horizon
        ref_traj = np.tile(ref_state.reshape(4, 1), (1, self.N + 1))
        return ref_traj
